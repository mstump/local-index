...
- **Embeddings**: Voyage AI API (not OpenAI; no local embedding models in the default pipeline)
- **Search reranking**: optional Anthropic Messages API (`ANTHROPIC_API_KEY`); no local reranker
...
- Vector search returns results with distance scores. Use cosine distance for the configured embedding model (Voyage voyage-3.5).
...
### HTTP APIs (Voyage + Anthropic)

| Technology | Version | Purpose                            | Why                                                                                                                                                                                 | Confidence                         |
|------------|---------|------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|
| reqwest    | ^0.12   | HTTP client for Voyage + Anthropic | Voyage: batch embeddings. Anthropic: Messages API for optional search reranking (`src/claude_rerank.rs`). Thin bespoke clients avoid third-party SDK drift for small surface areas. | HIGH (on approach; verify version) |
| serde      | ^1.0    | JSON serialization                 | Serialize/deserialize API request/response types.                                                                                                                                   | HIGH                               |
| serde_json | ^1.0    | JSON processing                    | Parse API responses (including rerank `{"indices":[...]}` from assistant text).                                                                                                     | HIGH                               |

- **Reranking:** After LanceDB retrieval (semantic, FTS, or hybrid with LanceDB RRF), the daemon/CLI/dashboard may call Claude with numbered candidate excerpts; the reply must be a single JSON object. Parsing tolerates a fenced code block around the JSON.
- Third-party Anthropic SDK crates are optional; Messages API + `reqwest` is sufficient for rerank.
...
Placeholder dependency groups (paste into `Cargo.toml` as TOML comments):
...
High level: **Walker → Chunker → VoyageEmbedder → ChunkStore (LanceDB)**; **SearchEngine** runs vector/FTS/hybrid queries on the table, then optionally **`AnthropicReranker`** (`claude_rerank` module) reorders a capped candidate pool via the Messages API when `ANTHROPIC_API_KEY` is set and `SearchOptions.rerank` is true. CLI flag `--no-rerank` and the dashboard query param `no_rerank` disable the call. Default rerank model is overridable with `LOCAL_INDEX_RERANK_MODEL`.
